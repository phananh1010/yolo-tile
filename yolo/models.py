...
class Darknet(nn.Module):
    ...
    def forward(self, x, targets=None):
        ...
        #ORIGINAL CODE, bring the code to CPU
        #yolo_outputs = to_cpu(torch.cat(yolo_outputs, 1))
        #yolo_outputs = [to_cpu(layer) for layer in layer_outputs]
        #yolo_outputs = to_cpu(layer_outputs[-1])
        
        
        x = layer_outputs[-1] 
        
        #ADDING customize prediction layers (not detection)
        #x = self.apool(x)
        #x = torch.flatten(x, 1)
        #x = self.fc1(x)
